...
def admin_account_detail(aid: int, admin=Depends(require_admin)):
    def _do():
        conn = db_conn()
        cur = conn.cursor()

        cur.execute("SELECT id, phone, verified, created_at, updated_at FROM accounts WHERE id=?", (int(aid),))
        acc = cur.fetchone()

        cur.execute(
            "SELECT id, ip, inicio, vence, estado, raw FROM proxies WHERE user_id=? ORDER BY id DESC LIMIT 50",
            (int(aid),),
        )
        proxies_rows = cur.fetchall()
        ...
        conn.close()
        return acc, proxies_rows, req_rows

    acc, proxies_rows, req_rows = _retry_sqlite(_do)
    if not acc:
        return nice_error_page("Cuenta", "No encontré esa cuenta.", "/admin/accounts", "⬅️ Volver")
    ...
    for r in req_rows:
        voucher = (r["voucher_path"] or "").strip()
        voucher_cell = f"<a href='/static/{html_escape(voucher)}' target='_blank'>ver</a>" if voucher else "-"
        extra = f" • Proxy #{int(r['target_proxy_id'])}" if (r["tipo"] == "renew" and int(r["target_proxy_id"] or 0) > 0) else ""
        ohtml += (
            "<tr>"
            f"<td>#{int(r['id'])}</td>"
    ...
    estado = "✅ Verificada" if int(acc["verified"] or 0) == 1 else "⏳ Pendiente"
    body = f"""
    <div class="card hero">
      <h1>👤 Cuenta #{int(acc['id'])}</h1>
      <p>Teléfono: <b>{html_escape(acc['phone'] or '')}</b> • Estado: <b>{estado}</b></p>
      <div class="hr"></div>
      <a class="btn ghost" href="/admin/accounts">⬅️ Volver</a>
    </div>

    <div class="card">
      <h3 style="margin:0 0 10px 0;">📦 Proxies</h3>
      <table>
        <tr><th>ID</th><th>IP</th><th>Vence</th><th>Estado</th></tr>
        {phtml}
      </table>
    </div>
    ...
    """
    return page(f"Admin • Cuenta {aid}", body, subtitle="Detalle")
...
def client_notifications(client=Depends(require_client)):
    ...
    # marcar visto automáticamente al abrir (si hay)
    if unseen_ids:
        def _mark():
            conn = db_conn()
            cur = conn.cursor()
...
# =========================
# SUPPORT (TICKETS) — NO "detail no autorizado"
# =========================
