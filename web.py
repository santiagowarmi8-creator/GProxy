...
async def on_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data.startswith("proxy_delete_"):
        pid = safe_int(data.split("_")[-1], 0)
        await handle_proxy_delete_start(caller, pid, context, query.message)
        return
    ...
    # ... (EL RESTO DE TU on_callback se queda IGUAL DESDE AQUÍ)
    # IMPORTANTE: No lo repetí para no hacerlo eterno, pero no cambies tu lógica existente.

    # Si llegas aquí sin match, no hagas nada
    return
